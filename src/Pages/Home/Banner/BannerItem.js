...
const BannerItem = ({ slide }) => {
  ...
  return (
    <div id={`slide${id}`} className="carousel-item min-w-full relative">
      <div className="carousel-img min-w-full relative">
        <img src={image} alt="" className="w-full h-[100vh] object-cover" />
        <div className="absolute w-full h-full bg-black top-0 bg-opacity-30"></div>
      </div>
      <div className="absolute flex justify-end transform -translate-y-1/2 left-24 top-1/3">
        <h1 className="text-5xl font-bold text-white">
          Welcome To
          <br /> Blogar Bangladesh
          <br />
        </h1>
      </div>
      ...
      <div className="absolute flex justify-start transform -translate-y-1/2 left-24 top-3/4 w-2/5">
        <Link to="/login" className="btn bg-orange-500  font-bold mr-5">Explore</Link>
      </div>
